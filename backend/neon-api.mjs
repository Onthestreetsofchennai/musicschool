...
import { hashPassword, normalizeSkillPayload, verifyPassword } from "./database.mjs";
...
function hashOtp(code, salt, secret) {
  return createHmac("sha256", secret).update(`${salt}:${code}`).digest("hex");
}

function otpMatches(code, salt, expectedHash, secret) {
  return hashOtp(code, salt, secret) === expectedHash;
}
...
export async function createNeonApi(context) {
  ...
  async function ensureBootstrap() {
    ...
    const teacherEmail = normalizeEmail(process.env.TEACHER_EMAIL);
    const teacherPassword = String(process.env.TEACHER_PASSWORD || "");
    if (!adminEmail || !adminPassword || !teacherEmail || !teacherPassword) {
      throw new Error("ADMIN_EMAIL, ADMIN_PASSWORD, TEACHER_EMAIL and TEACHER_PASSWORD are required with DATABASE_URL");
    }
    ...
    `, [process.env.ADMIN_NAME || "MUSIC SCHOOL Admin", adminEmail, hashPassword(adminPassword)]);

    const teacherUser = await one(`
      INSERT INTO users (name, email, password_hash, role)
      VALUES ($1, $2, $3, 'teacher')
      ON CONFLICT (email) DO UPDATE SET name = EXCLUDED.name
      RETURNING id
    `, [process.env.TEACHER_NAME || "MUSIC SCHOOL Teacher", teacherEmail, hashPassword(teacherPassword)]);
    const teacherUserId = teacherUser?.id || (await one("SELECT id FROM users WHERE email = $1", [teacherEmail]))?.id;
    await query(`
    ...
    `, [
      teacherUserId,
      process.env.TEACHER_INSTRUMENT || "Guitar",
      "Guided music teacher for the 12-week MUSIC SCHOOL program."
    ]);

    const course = await one(`
      INSERT INTO courses (name, instrument, duration_weeks)
      SELECT $1, $2, 12
      WHERE NOT EXISTS (SELECT 1 FROM courses WHERE instrument = $2 AND active = TRUE)
      RETURNING id
    `, [
      `12-Week ${process.env.TEACHER_INSTRUMENT || "Guitar"} Foundations`,
      process.env.TEACHER_INSTRUMENT || "Guitar"
    ]);
    const courseId = course?.id || (await one(
    ...
    for (let index = 0; index < weekTitles.length; index += 1) {
      await query(`
  ...
  async function authSession(request) {
    ...
    const row = await one(`
      SELECT auth.*, u.name AS staff_name, u.email AS staff_email,
        s.name AS student_name, account.email AS student_email
  ...
  async function handle(request, response, url) {
    ...
    if (pathname === "/api/health" && request.method === "GET") {
      return sendJson(response, 200, {
        ...
        emailDelivery: process.env.CLOUDFLARE_EMAIL_WORKER_URL ? "cloudflare" : "unconfigured",
        videoStorage: cloudinaryIsConfigured() ? "cloudinary-private" : "unconfigured",
    ...
    if (pathname === "/api/student-auth/request-otp" && request.method === "POST") {
      ...
      if (!account) return sendJson(response, 200, { ok: true, message: generic });
      const recent = await one(`
        SELECT COUNT(*)::int AS count FROM otp_challenges
        WHERE student_account_id = $1 AND created_at >= NOW() - INTERVAL '10 minutes'
      `, [account.id]);
      if (number(recent?.count) >= 3) return sendJson(response, 429, { error: "Too many codes requested. Please wait 10 minutes." });
      ...
      const challenge = await one(`
        INSERT INTO otp_challenges (student_account_id, code_hash, code_salt, expires_at)
        VALUES ($1, $2, $3, NOW() + INTERVAL '10 minutes') RETURNING id
      `, [account.id, hashOtp(code, salt, otpSecret), salt]);
      try {
        ...
        return sendJson(response, 200, { ok: true, message: "Login code sent to your email.", expiresInSeconds: 600 });
      } catch (error) {
    ...
    if (pathname === "/api/student-auth/verify-otp" && request.method === "POST") {
      ...
      if (!isValidEmail(email) || !/^\d{6}$/.test(code)) return sendJson(response, 400, { error: "Enter the six-digit login code" });
      const account = await one(`
        SELECT account.*, s.name AS student_name FROM student_accounts account
        JOIN students s ON s.id = account.student_id
        WHERE account.email = $1 AND account.active = TRUE AND s.active = TRUE
      ...
      const challenge = await one(`
        SELECT * FROM otp_challenges WHERE student_account_id = $1
      ...
      if (!otpMatches(code, challenge.code_salt, challenge.code_hash, otpSecret)) {
