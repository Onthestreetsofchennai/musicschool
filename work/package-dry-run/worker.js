...
import { createHash, createHmac, randomBytes as randomBytes2, randomInt, randomUUID } from "node:crypto";

// backend/shared.mjs
import { randomBytes, scryptSync, timingSafeEqual } from "node:crypto";
var SKILL_FIELDS = ["rhythm", "accuracy", "technique", "posture", "musicality", "confidence"];
...
__name(normalizeSkillPayload, "normalizeSkillPayload");

// backend/neon-api.mjs
var OTP_EXPIRY_MINUTES = 5;
var STUDENT_SESSION_DAYS = 7;
var STAFF_SESSION_HOURS = 12;
var COURSE_WEEK_TITLES = [
  "Setup, posture and first sound",
  "Pulse and rhythm foundations",
  "First chord shapes",
  "Clean transitions",
  "Practice patterns",
  "First complete song",
...
__name(statusRank, "statusRank");
async function createNeonApi(context) {
  ...
  __name(ensureCourse, "ensureCourse");
  ...
  async function createSession({ principalType, userId = null, studentId = null, role, ttlMilliseconds }) {
    ...
    const token = signSessionToken({
      ...
      nonce: randomBytes2(16).toString("base64url")
    });
    await query(`
  ...
  async function authSession(request) {
    ...
    if (claims.principalType !== row.principal_type || String(claims.userId || "") !== String(row.user_id || "") || String(claims.studentId || "") !== String(row.student_id || "") || claims.role !== row.role) return null;
  ...
  async function requireStaff(request, response, roles = []) {
    ...
    if (!session || session.principalType !== "staff") {
      sendJson2(response, 401, { error: "Authentication required" });
      return null;
    }
    if (roles.length && !roles.includes(session.role)) {
      sendJson2(response, 403, { error: "Insufficient permission" });
      return null;
    }
    return session;
  }
  ...
  async function requireStudent(request, response) {
    ...
    if (!session || session.principalType !== "student" || !session.studentId) {
      sendJson2(response, 401, { error: "Student login required" });
      return null;
    }
    return session;
  }
  ...
  __name(teacherScope, "teacherScope");
  async function recalculateStudent(studentId) {
    ...
    `, [studentId]);
    const practiceScore = clamp2(number(submission?.count) / 14 * 100, 0, 100);
  ...
  __name(recalculateStudent, "recalculateStudent");
  ...
  __name(studentAnalysis, "studentAnalysis");
  ...
  __name(todaySubmissions, "todaySubmissions");
  function practiceGate(submissions) {
    ...
    const submitted = new Set(submissions.filter((item) => number(item.duration_seconds) >= minPracticeSeconds).map((item) => item.period));
    ...
    return {
      ...
      message: activePeriod ? `${activePeriod === "morning" ? "Morning" : "Evening"} practice is required before using the app. Upload at least ${Math.round(minPracticeSeconds / 60)} minutes.` : "Practice gate is clear for now."
    };
  }
  __name(practiceGate, "practiceGate");
  ...
  async function handle(request, response, url) {
    ...
    if (pathname === "/api/health" && request.method === "GET") {
      return sendJson2(response, 200, {
        status: "ok",
        database: "neon-postgresql",
        emailDelivery: environment.RESEND_API_KEY && environment.EMAIL_FROM ? "resend" : "unconfigured",
    ...
    if (pathname === "/api/student-auth/request-otp" && request.method === "POST") {
      ...
      if (!isValidEmail(email)) return sendJson2(response, 400, { error: "Enter a valid email address" });
      ...
      if (!account) {
        return sendJson2(response, 200, {
          ...
          expiresAt: new Date(Date.now() + OTP_EXPIRY_MINUTES * 6e4).toISOString(),
      ...
      if (number(recent?.count) >= 5) {
        return sendJson2(response, 429, { error: "Too many codes requested. Please try again after one hour." });
      }
      if (recent?.latest_created_at && Date.now() - new Date(recent.latest_created_at).getTime() < 6e4) {
        return sendJson2(response, 429, { error: "Please wait 60 seconds before requesting another code." });
      }
      ...
      try {
        ...
        console.error(error);
        return sendJson2(response, 503, { error: "The login email could not be sent. Please contact support." });
      }
    }
    if (pathname === "/api/student-auth/verify-otp" && request.method === "POST") {
      ...
      if (!isValidEmail(email) || !otpSessionId || !/^\d{6}$/.test(code)) {
        return sendJson2(response, 400, { error: "Enter the six-digit login code" });
      }
      const account = await one(`
        SELECT account.*, s.name AS student_name FROM student_accounts account
        JOIN students s ON s.id = account.student_id
        WHERE account.email = $1 AND account.active = TRUE AND s.active = TRUE
      `, [email]);
      if (!account) return sendJson2(response, 401, { error: "Invalid or expired login code" });
      const challenge = await one(`
      ...
      if (!challenge || new Date(challenge.expires_at) <= /* @__PURE__ */ new Date() || number(challenge.attempts) >= number(challenge.max_attempts)) {
        return sendJson2(response, 401, { error: "Invalid or expired login code" });
      }
      ...
      const auth = await createSession({
        ...
        role: "student",
        ttlMilliseconds: STUDENT_SESSION_DAYS * 864e5
      });
    ...
    if (pathname === "/api/student-auth/me" && request.method === "GET") {
      ...
      if (!session) return;
      return sendJson2(response, 200, { student: { id: session.studentId, name: session.name, email: session.email }, expiresAt: session.expiresAt });
    }
    if (pathname === "/api/student-auth/logout" && request.method === "POST") {
      ...
      return sendJson2(response, 200, { ok: true });
    }
    if (pathname === "/api/auth/login" && request.method === "POST") {
      const body = await readJson2(request);
      const user = await one("SELECT * FROM users WHERE email = $1 AND active = TRUE", [normalizeEmail(body.email)]);
      if (!user || !verifyPassword(String(body.password || ""), user.password_hash)) {
        return sendJson2(response, 401, { error: "Invalid email or password" });
      }
      const auth = await createSession({
        principalType: "staff",
        userId: user.id,
        role: user.role,
        ttlMilliseconds: STAFF_SESSION_HOURS * 36e5
      });
    ...
    if (pathname === "/api/auth/me" && request.method === "GET") {
      ...
      if (!session) return;
      return sendJson2(response, 200, { user: session });
    }
    ...
    if (pathname === "/api/dashboard" && request.method === "GET") {
      ...
      if (!session) return;
      return sendJson2(response, 200, await dashboard(session));
    }
    if (pathname === "/api/teachers" && request.method === "GET") {
      ...
      `);
      return sendJson2(response, 200, { teachers });
    }
    if (pathname === "/api/staff" && request.method === "GET") {
      ...
      `);
      return sendJson2(response, 200, { staff });
    }
    if (pathname === "/api/staff" && request.method === "POST") {
      ...
      if (name.length < 2 || !isValidEmail(email) || password.length < 8 || !allowedRoles.includes(role)) {
        return sendJson2(response, 400, { error: "Name, valid email, role and a password of at least 8 characters are required" });
      }
      if (role === "teacher" && !instrument) {
        return sendJson2(response, 400, { error: "Instrument is required for a teacher" });
      }
      if (await one("SELECT id FROM users WHERE email = $1", [email])) {
        return sendJson2(response, 409, { error: "A staff account already uses this email" });
      }
    ...
    if (staffStatusMatch && request.method === "PATCH") {
      ...
      const target = await one(`
        ...
        WHERE u.id = $1
      `, [staffId]);
      if (!target) return sendJson2(response, 404, { error: "Staff account not found" });
      if (staffId === session.userId && !active) {
        return sendJson2(response, 400, { error: "You cannot deactivate your own account" });
      }
      if (!active && target.teacher_id) {
        ...
        if (number(assigned?.count) > 0) {
          return sendJson2(response, 409, { error: "Reassign this teacher's active students before deactivating the account" });
        }
      }
      ...
      return sendJson2(response, 200, { ok: true, active });
    }
    if (pathname === "/api/students" && request.method === "GET") {
      ...
      students = students.filter((row) => !status || row.status === status).filter((row) => !teacherId || String(row.teacher_id) === teacherId).filter((row) => !search || `${row.name} ${row.instrument} ${row.teacher_name}`.toLowerCase().includes(search)).sort((a2, b2) => statusRank(a2.status) - statusRank(b2.status) || number(a2.overall_score) - number(b2.overall_score));
      return sendJson2(response, 200, { students });
    }
    if (pathname === "/api/students" && request.method === "POST") {
      ...
      if (name.length < 2 || !isValidEmail(email) || !body.instrument || !body.goal || !body.ageGroup || !teacherId) {
        return sendJson2(response, 400, { error: "Name, email, age group, instrument, goal and teacher are required" });
      }
      if (parentEmail && !isValidEmail(parentEmail)) return sendJson2(response, 400, { error: "Enter a valid parent email address" });
      if (await one("SELECT id FROM student_accounts WHERE email = $1", [email])) {
        return sendJson2(response, 409, { error: "A student account already uses this email" });
      }
      const teacher = await one("SELECT id, instrument FROM teachers WHERE id = $1 AND active = TRUE", [teacherId]);
      if (!teacher || teacher.instrument !== body.instrument) return sendJson2(response, 400, { error: "Choose a matching active teacher" });
      const course = await one("SELECT id FROM courses WHERE instrument = $1 AND active = TRUE ORDER BY id LIMIT 1", [body.instrument]);
      if (!course) return sendJson2(response, 400, { error: "No active course exists for this instrument" });
      const student = await one(`
      ...
      `, [
        ...
        teacherId,
        body.courseStartDate || (/* @__PURE__ */ new Date()).toISOString().slice(0, 10),
      ...
      return sendJson2(response, 201, { student: await studentAnalysis(student.id) });
    }
    ...
    if (studentMatch && request.method === "GET") {
      ...
      if (scope !== null && !await one("SELECT id FROM students WHERE id = $1 AND assigned_teacher_id = $2", [studentId, scope])) {
        return sendJson2(response, 403, { error: "Student is outside your assigned roster" });
      }
      const analysis = await studentAnalysis(studentId);
      return analysis ? sendJson2(response, 200, analysis) : sendJson2(response, 404, { error: "Student not found" });
    }
    if (pathname === "/api/reviews" && request.method === "GET") {
      ...
      `, [status, scope]);
      return sendJson2(response, 200, { submissions });
    }
    ...
    if (reviewMatch && request.method === "POST") {
      ...
      if (!submission) return sendJson2(response, 404, { error: "Submission not found" });
      const scope = await teacherScope(session);
      if (scope !== null && number(submission.teacher_id) !== scope) return sendJson2(response, 403, { error: "Submission is outside your queue" });
      if (submission.review_status === "reviewed") return sendJson2(response, 409, { error: "Submission already reviewed" });
      const body = await readJson2(request);
      ...
      `, [
        ...
        body.positiveObservation || "Good focused practice.",
      ...
      return sendJson2(response, 201, { ok: true, student: await studentAnalysis(submission.student_id) });
    }
    ...
    if (pathname === "/api/alerts" && request.method === "GET") {
      ...
      `, [scope]);
      return sendJson2(response, 200, { alerts });
    }
    ...
    if (pathname === "/api/student/me" && request.method === "GET") {
      ...
      if (!analysis) return sendJson2(response, 404, { error: "Student not found" });
      ...
      return sendJson2(response, 200, {
        ...
        upcomingSessions: analysis.sessions.filter((item) => item.status === "scheduled" && new Date(item.scheduled_at) >= /* @__PURE__ */ new Date()).sort((a2, b2) => new Date(a2.scheduled_at) - new Date(b2.scheduled_at)).slice(0, 4),
    ...
    if (pathname === "/api/student/me/practice-submissions" && request.method === "POST") {
      ...
      if (!["morning", "evening"].includes(body.period)) return sendJson2(response, 400, { error: "Invalid practice period" });
      if (durationSeconds < minPracticeSeconds) return sendJson2(response, 400, { error: "Practice video must be at least 7 minutes." });
      if (await one(`
        SELECT id FROM practice_submissions WHERE student_id = $1 AND period = $2
          AND (uploaded_at AT TIME ZONE 'Asia/Kolkata')::date = (NOW() AT TIME ZONE 'Asia/Kolkata')::date
      `, [session.studentId, body.period])) return sendJson2(response, 409, { error: `${body.period} practice was already submitted today` });
      const submission = await one(`
      ...
      `, [
        ...
        `metadata/students/${session.studentId}/${body.period}-${Date.now()}-${randomBytes2(6).toString("hex")}`
      ]);
    ...
    if (pathname === "/api/student/me/help-calls" && request.method === "POST") {
      ...
      `, [
        session.studentId,
        student.assigned_teacher_id,
        body.topic || "Student requested support",
        body.scheduledAt || new Date(Date.now() + 864e5).toISOString(),
        `ots-help-${session.studentId}-${Date.now()}`
      ]);
      return sendJson2(response, 201, { id: number(call.id), scheduledAt: call.scheduled_at, status: "scheduled" });
    }
    ...
    if (cancelCall && request.method === "POST") {
      ...
      return rows.length ? sendJson2(response, 200, { ok: true }) : sendJson2(response, 404, { error: "Scheduled help call not found" });
    }
    if (pathname === "/api/student/me/progress" && request.method === "POST") {
      const session = await requireStudent(request, response);
      if (!session) return;
      const body = await readJson2(request);
      const week = clamp2(number(body.currentWeek, 1), 1, 12);
      await query("UPDATE students SET current_week = $1 WHERE id = $2", [week, session.studentId]);
      await recalculateStudent(session.studentId);
      return sendJson2(response, 200, { ok: true, currentWeek: week });
    }
    if (pathname === "/api/student/me/profile" && request.method === "PATCH") {
      ...
      if (name.length < 2 || goal.length < 3) return sendJson2(response, 400, { error: "Name and learning goal are required" });
      await query("UPDATE students SET name = $1, goal = $2 WHERE id = $3", [name, goal, session.studentId]);
      return sendJson2(response, 200, { ok: true, profile: (await studentAnalysis(session.studentId)).student });
    }
    ...
    return sendJson2(response, 404, { error: "API route not found" });
  }
  __name(handle, "handle");
  return handle;
}
__name(createNeonApi, "createNeonApi");
...
function createSessionSigner(secret) {
  function signSessionToken(payload) {
    ...
    const signature = createHmac2("sha256", secret).update(encodedPayload).digest("base64url");
  ...
  function verifySessionToken(token) {
    ...
    const expectedSignature = createHmac2("sha256", secret).update(encodedPayload).digest();
    const suppliedBuffer = Buffer.from(suppliedSignature, "base64url");
    if (suppliedBuffer.length !== expectedSignature.length || !timingSafeEqual2(suppliedBuffer, expectedSignature)) {
      return null;
    }
    try {
...
__name(createSessionSigner, "createSessionSigner");
function escapeHtml(value) {
  return String(value).replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;").replaceAll("'", "&#039;");
}
__name(escapeHtml, "escapeHtml");
function createOtpDelivery(environment) {
  return /* @__PURE__ */ __name(async function deliverOtpEmail({ email, studentName, code, challengeId }) {
...
__name(createOtpDelivery, "createOtpDelivery");
...
__name(createWorkerApi, "createWorkerApi");
var apiServer = createServer(async (request, response) => {
  try {
